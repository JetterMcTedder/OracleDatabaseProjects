...
resource "null_resource" "sqlcl-load-data" {
        ...
        provisioner "local-exec" {    
            command = <<-EOT
                sql -cloudconfig wallet.zip admin/${random_string.password.result}@REDBDB_high @./db_scripts/redbull.sql
            EOT
        }
...
output "dashboard_url" {
  ...
  value = replace("${oci_database_autonomous_database.strava_autonomous_database.connection_urls.0.sql_dev_web_url}admin/_sdw/dashboards/?name=Red%20Bull%20Race%20Data","sql-developer","")
